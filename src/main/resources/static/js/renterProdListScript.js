...
function display_products(productList) {
	...
	for (i = 0; i < pdt_row_length; i++) // loop for no of rows
	{
		...
		for (j = 0; j < 3; j++) // 3 items per row
		{
			...
			$("div.pdt-img" + (pdt_index + 1)).append("<a href='/renterProductDetails.html?Product_ID=" + productList[pdt_index].productId + "&Product_Name="+productList[pdt_index].productName+"'><img src=\"data:image/jpg;base64," + productList[pdt_index].prodImg + "\" style='object-fit: contain'/></a>");
			...
			var ratingRandom = Math.floor(Math.random() * 5) + 1;
